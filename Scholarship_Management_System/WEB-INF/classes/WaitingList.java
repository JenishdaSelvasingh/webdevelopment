...
String s = String.format("select * from status where ticstatus='Waiting'");
ResultSet rs = st.executeQuery(s);

wlist = "<table border='2' cellspacing='3' cellpadding='30' bordercolor='#9a00ff' style='margin-left: 320px; margin-top: 30px;'><tr><th>Ticket No.</th><th>Passenger Name</th><th>Train Number</th><th>Train Name</th><th>Source</th><th>Destination</th><th>Date of Travel</th><th>No of Seats</th></tr>";
while(rs.next()) {
...
record += "<tr>";
record += "<td>" + Integer.toString(ticno) + "</td>"; //student no
record += "<td>" + pname + "</td>"; // student name
record += "<td>" + Integer.toString(tid) + "</td>"; //scholarship id
record += "<td>" + rs2.getString(1) + "</td>"; // eligibility
record += "<td>" + rs2.getString(2) + "</td>"; // scholarship name
record += "<td>" + rs2.getString(3) + "</td>"; // community
record += "<td>" + dot + "</td>"; // role
record += "<td>" + Integer.toString(asno) + "</td>"; // aadhar number
record += "</tr>";
rs2.close();
wlist += record;
